...
export default function EditCategories() {
    ...
    const loadData = async () => {
      try {
        ...
        setForm({
          ...
          isActive: isCatActive(data),
        });
      } catch {
        toast.error('Không tải được thông tin danh mục');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await api.put(`/categories/${id}`, buildFormData());
    ...
    return (
      <div className={`flex min-h-screen items-center justify-center ${dark ? 'bg-slate-950' : 'bg-slate-50'}`}>
        <div className="text-center">
          ...
          <p className={`text-sm ${txt3}`}>Đang tải thông tin danh mục...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex gap-6">
          ...
          <aside className="hidden lg:flex w-52 shrink-0 flex-col gap-1 self-start sticky top-6">
            ...
            <div className={`mt-4 rounded-2xl border p-3 ${dark ? 'border-slate-800 bg-slate-900/50' : 'border-slate-200 bg-slate-50'}`}>
              <p className={`text-[10px] uppercase tracking-widest font-semibold ${txt3}`}>Mã danh mục</p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className={`mt-0.5 text-xs ${txt3}`}>
                      {form.isActive ? 'Khách hàng có thể xem danh mục này' : 'Danh mục đang bị ẩn khỏi cửa hàng'}
                    </p>
                  </div>
                </div>
