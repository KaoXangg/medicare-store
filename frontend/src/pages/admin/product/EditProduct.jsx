...
function specsFromProduct(data) {
  if (!data?.Specifications) return DEFAULT_SPECS;
  try {
    const parsed = typeof data.Specifications === 'string'
      ? JSON.parse(data.Specifications)
      : data.Specifications;
    const entries = Object.entries(parsed || {});
    return entries.length ? entries.map(([key, value]) => ({ key, value: String(value) })) : DEFAULT_SPECS;
...
function CategorySelect({ value, onChange, categories, dark }) {
    ...
    const loadData = async () => {
      try {
        const [catRes, brandRes, productRes] = await Promise.all([
          api.get('/categories?active=false'),
          api.get('/brands'),
        ...
        setForm({
          ...
          stock: data.Stock || '',
          // Sản phẩm cũ chưa có SKU sẽ được tự động gán mã ngẫu nhiên
        ...
        setSpecRows(specsFromProduct(data));
        setImageUrls((data.images || []).map((i) => i.ImageUrl).join('\n'));
      } catch {
    ...
    return (
      <div className={`flex min-h-screen items-center justify-center ${dark ? 'bg-slate-950' : 'bg-slate-50'}`}>
        <div className="text-center">
          ...
          <p className={`text-sm ${txt3}`}>Đang tải thông tin sản phẩm...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className={`text-[10px] uppercase tracking-[0.22em] font-semibold ${txt3}`}>
                Chỉnh sửa · #{id}
              </p>
              <h1 className={`text-xl font-black truncate max-w-xs sm:max-w-md ${txt1}`}>
        ...
        <div className="flex gap-6">
          ...
          <aside className="hidden lg:flex w-52 shrink-0 flex-col gap-1 self-start sticky top-6">
            ...
            {/* Product ID badge */}
            <div className={`mt-4 rounded-2xl border p-3 ${dark ? 'border-slate-800 bg-slate-900/50' : 'border-slate-200 bg-slate-50'}`}>
              <p className={`text-[10px] uppercase tracking-widest font-semibold ${txt3}`}>Mã sản phẩm</p>
              <p className={`mt-1 text-sm font-black ${txt1}`}>#{id}</p>
            </div>
          </aside>
          ...
          <form onSubmit={handleSubmit} className="flex-1 min-w-0 space-y-4">
            ...
            <SectionCard id="media" icon={ImageIcon} title="Hình ảnh sản phẩm" subtitle={`Tối đa ${MAX_IMAGES} ảnh upload · Hỗ trợ link URL`} dark={dark}>
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div>
                  <FieldLabel dark={dark}>Upload ảnh mới</FieldLabel>
